-- 하하하 프로필을 유하진 조 명단에 직접 매칭하고 H&P 입력 정답 정책을 재고정한다.
begin;
...
declare
  ...
  v_previous_target_profile_id uuid := null;
...
begin
  ...
  where p.account_status = 'active'
    and (
      p.login_id::text = '하하하'
      or nullif(btrim(coalesce(p.display_name, '')), '') = '하하하'
    );

  if v_profile_count <> 1 then
    raise exception 'expected exactly one active profile with nickname/login_id 하하하, got %', v_profile_count;
  end if;
  ...
  set matched_profile_id = v_profile.id,
      ...
      match_detail = case
        when v_unmatched > 0 then '닉네임 하하하 프로필을 유하진으로 보정하고 기존 연결을 이동'
        else '닉네임 하하하 프로필을 유하진으로 보정해 수동 매칭'
      end,
      updated_at = now()
  where r.id = v_roster.id;

  if v_previous_target_profile_id is not null
     and v_previous_target_profile_id is distinct from v_profile.id then
    delete from public.bbb_assignments
    where profile_id = v_previous_target_profile_id;
  ...
  perform public.bu_sync_bbb_assignments_from_roster('20260614');
