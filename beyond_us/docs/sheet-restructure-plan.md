...
> 위험도 차이가 크므로 2A~2E로 단계 분리. 각 단계 끝마다 DEV에서 며칠 모니터링 후 다음 단계.
...
|-----|------|----------|
| A | userId | `=Users!A2` |
| B | 닉네임 | `=VLOOKUP(A2, Users!A:E, 2, 0)` |
| C | 교구 | `=VLOOKUP(A2, Users!A:E, 4, 0)` |
| D | 미션수 | `=COUNTIFS(Events!C:C, A2, Events!D:D, "mission.submitted")` |
| E | 티켓획득 | `=SUMPRODUCT((Events!C:C=A2)*(Events!D:D="ticket.granted")*regex_extract(...))` |
| F | 뽑은수 | `=COUNTIFS(Events!C:C, A2, Events!D:D, "card.drawn")` |
| G | 남은권 | `=E2-F2` |
| H | 마지막활동 | `=MAXIFS(Events!B:B, Events!C:C, A2)` |
| I | Collection_남은권 | `=VLOOKUP(A2, Collection!A:Z, 4, 0)` |
| J | 일치? | `=IF(G2=I2, "✓", "❌")` |

`payload` JSON에서 `amount` 추출은 `REGEXEXTRACT` 또는 `LAMBDA` 활용. 정확한 함수는 2.0에서 확정.

---

### 2A — Events 시트 + Dual-Write [non-breaking]

> 기존 시트에 쓰는 동시에 Events에도 append. 기존 동작 변경 없음. 안전.

**작업.**
- DEV 시트에 `Events` 시트 추가 + 헤더 + 프리징.
- `Events.append(type, userId, payload, source)` 헬퍼 작성. UUID + timestamp 자동 생성. `LockService`로 동시성 보호.
- 모든 mutation 코드에 dual-write 패치.
  ```js
  // 기존
  rawCheckinsSheet.appendRow([...]);
  // 변경 후 — 기존 유지 + Events append
  rawCheckinsSheet.appendRow([...]);
  Events.append('mission.submitted', userId, {weekKey, items, dateKey}, 'web');
  ```

**검증.**
- DEV에서 며칠 사용 후 Events vs 기존 시트 행 수 비교.
- 비교 스크립트 (`verifyDualWrite()`) — 모든 mission/draw/trade 이벤트가 양쪽에 있는지.

**산출.**
- Events 시트가 truth source 후보로 모든 mutation을 기록.
...
**검증.**
- 모든 행이 ✓ 인지 확인. ❌ 가 있다면 Phase 2A dual-write에 누락 있음 → 추적해서 수정.
...
> 시트 분리 / 헤더 정규화 / 하드코딩 외부화. Events는 이미 truth source라 raw_checkins/CardDraws/BonusDraws 흡수가 안전.
...
**중요.** `migrate_step5_absorbToEvents`는 과거 데이터를 Events에 backfill하는 1회성 작업. 2A부터 dual-write로 적힌 새 이벤트와 timestamp 순으로 통합. eventId는 새로 부여하되 source는 `migration`.
...
| 리스크 | 단계 | 영향 | 확률 | 완화책 |
|---|---|---|---|---|
| Events.append 실패로 mutation 손실 | 2A/3A | 높음 | 낮음 | LockService + try/catch 재시도. dual-write라 기존 시트엔 남음 |
...
| Phase 2A | 1~2일 + 며칠 모니터링 | dual-write 패치. 위험 낮음 |
...
- 5/15~5/17 — Phase 2A DEV 적용 + 며칠 모니터링
