...
- `SPREADSHEET_ID`, `DEV_SPREADSHEET_ID`, `ADMIN_PASSWORD`는 Apps Script Properties로 이동.
- GAS 코드에는 `PropertiesService.getScriptProperties().getProperty('KEY')` 형태의 접근 함수만 남김.
- DEV와 PROD는 같은 코드라도 배포별 Properties 값만 다르게 설정.
- 코드 안에는 민감값 fallback을 남기지 않고, Properties 누락 시 명시적인 설정 오류를 반환.
- DEV Apps Script 프로젝트에 Properties를 먼저 설정한 뒤 DEV 배포 URL만 새 버전으로 갱신.
- PROD Apps Script 프로젝트의 Properties 설정과 배포는 모든 Phase 완료 후 Phase 3에서 진행.
