...
- 여기서 말하는 미보유 카드 지급권은 BBB MISSION 1, 2 승인과 시크릿버디 정답 보상에 쓰인다.
- BBB MISSION 1, 2가 성공하면 각각 미보유 엔 카드 지급권을 1장씩 자동 지급한다.
- 시크릿버디 정답 최초 성공 시에도 미보유 엔 카드 지급권을 1장 지급한다.
- BBB MISSION 3 / 천로역정 7스팟 완료 보상은 미보유 카드 지급권이 아니라 레어 엔 카드(id=10) 직접 지급으로 변경됐다.
...
- 시크릿버디 정답 보상도 `bbb_secret_guess` source의 `special_pack.granted`로 기록한다.
- BBB MISSION 3은 7곳 사진이 모두 채워진 시점에 `card.granted`로 레어 엔 카드(id=10)를 직접 지급한다.
- 기존 BBB 보상으로 남아 있는 `ticket.granted` 이벤트는 중복 지급 방지와 보안 감사에서 계속 인식한다.

## 확정 정책

- 모든 일반 카드를 보유한 유저의 특별 카드팩은 일반 카드팩처럼 1-9번 일반 카드 중 랜덤 지급한다.
- 레어 엔 카드는 랜덤 뽑기 대상이 아니라 천로역정 완료 보상으로 직접 지급한다.
