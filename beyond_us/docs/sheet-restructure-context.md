...
**제약.**
- **GAS는 이 시트에 쓰지 않는다.** 시트 함수만 사용. GAS 코드 변경 없음.
- `Events` 시트가 Phase 2A dual-write로 채워진 상태에서 동작하는 설계.
...
|---|---|---|
| A | `eventId` | |
| B | `timestamp` | ISO 8601 문자열 (MAXIFS 기준) |
| C | `userId` | 닉네임 (COUNTIFS/SUMIFS 기준) |
| D | `type` | `mission.submitted`, `ticket.granted`, `ticket.consumed`, `card.drawn`, … |
| E | `weekKey` | `w1`~`w6` |
| F | `refId` | card.drawn → 카드 ID (문자열 "1"~"10") |
| G | `amount` | 양수. granted=지급량, consumed=차감량 (항상 양수로 저장) |
...
> A열 `=Users!A3` 방식은 Users에 행이 추가되면 자동 반영. UserDashboard 행 수는 Users 행 수와 동일하게 유지.
...
| 컬럼 | machine header | 운영진 라벨 | 시트 함수 (Row 3) |
|---|---|---|---|
| E | `ticketsEarned` | 누적 획득 뽑기권 | `=SUMIFS(Events!G3:G,Events!C3:C,A3,Events!D3:D,"ticket.granted")` |
| F | `ticketsConsumed` | 사용 뽑기권 | `=SUMIFS(Events!G3:G,Events!C3:C,A3,Events!D3:D,"ticket.consumed")` |
...
| I | `card_1` | 사랑 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"1")` |
| J | `card_2` | 희락 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"2")` |
| K | `card_3` | 화평 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"3")` |
| L | `card_4` | 오래참음 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"4")` |
| M | `card_5` | 자비 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"5")` |
| N | `card_6` | 양선 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"6")` |
| O | `card_7` | 충성 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"7")` |
| P | `card_8` | 온유 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"8")` |
| Q | `card_9` | 절제 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"9")` |
| R | `card_10` | 히든 | `=COUNTIFS(Events!C3:C,A3,Events!D3:D,"card.drawn",Events!F3:F,"10")` |
...
| 컬럼 | machine header | 운영진 라벨 | 시트 함수 (Row 3) |
|---|---|---|---|
| U | `lastActivity` | 마지막 활동 | `=IFERROR(TEXT(MAXIFS(Events!B3:B,Events!C3:C,A3),"YYYY-MM-DD HH:MM:SS"),"")` |

> `Events!B` (timestamp)는 ISO 8601 문자열. MAXIFS로 알파벳 최대값(= 가장 최근 타임스탬프)을 구한다.  
> Google Sheets의 MAXIFS는 텍스트에도 동작하므로 ISO 8601 포맷이라면 정확히 동작.
...
5. A3~A252 (250명 기준): =Users!A3 ~ =Users!A252
