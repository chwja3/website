...
- 지도 탭은 `전체배치도`, `예루살렘동`, `갈릴리동` 순서로 배치했다.
- 갈릴리동 탭 안에서는 `갈릴리동34층.jpg`를 먼저 보여주고, 아래에 `갈릴리동12층.jpg`를 이어서 보여준다.
- `app.js` 캐시 버전은 `20260616a`로 올렸고, 갈릴리동 통합 CSS 반영을 위해 `app.css` 캐시 버전은 `20260616b`로 올렸다.
