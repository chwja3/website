...
- 조별 매칭 운영은 같은 조 안에서도 참여구분을 맞춰야 한다. `전참`, `토참`, `일참` 세 묶음으로 보여주고, 후보 선택도 같은 묶음 안으로 제한한다.
- `participationTier`가 아직 비어 있으면 화면에서는 `전참`으로 둔다. 데이터가 채워지면 그 값이 우선한다.
- `20260617000200_admin_bbb_matching_tier_groups.sql`에서 `bu_bbb_matching_tier()`를 추가했고, admin 저장 RPC도 다른 조 또는 다른 참여구분으로 저장하려 하면 거절한다.
