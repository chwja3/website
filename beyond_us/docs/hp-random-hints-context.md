...
2026-05-19. 힌트 row에 `hold_pray_entry_id`를 추가했다. 기존 구조는 `card_index`만 저장해서 랜덤 3장 기준이 바뀌면 오래된 힌트가 다른 카드에 붙을 수 있었기 때문이다. 현재는 초기 응답에 초성을 싣지 않지만, 힌트 요청 기록을 실제 H&P entry에 귀속시키기 위해 유지한다.

2026-05-19. 운영 기준을 재확인했다. 초성은 처음부터 주는 정보가 아니라 사용자가 힌트 요청 버튼을 누를 때만 받아야 한다. 따라서 `get_hold_pray`는 `hintReplies`를 항상 빈 객체로 반환하고, 앱도 H&P 캐시에 초성을 저장하지 않는다. `post_hold_pray_hint`만 초성을 즉시 반환한다.
