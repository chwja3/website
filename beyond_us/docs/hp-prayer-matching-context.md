...
- 같은 실명으로 활성 유저가 여러 명이면 후보 닉네임, 이름, 교구를 내려주고 admin에서 닉네임을 선택해 확정 매칭한다.
...
- 동명이인 상태에서는 이름 입력만으로 저장하지 않고 후보 닉네임 select를 보여준다. 선택된 `login_id`로 `profile_id`를 확정한다.
- 매칭 저장 후 정답 여부를 재계산할 때 `bu_hold_pray_cards_for_profile(uuid, text)` helper가 필요하다. 일부 Supabase 프로젝트에서 이 helper가 먼저 생성되어 있지 않으면 저장 실패가 나므로 `20260520000600_hp_prayer_matching_helper_guard.sql`에서 helper와 재계산 RPC를 다시 보강한다.
