...
기존 가입자는 신규 가입 흐름을 다시 통과하지 않기 때문에 `signup` 추첨권이 비어 있을 수 있다. DEV에서 `backfillSignupRaffleTicketsDryRun()` 후 `backfillSignupRaffleTicketsApply()`를 실행하면 발급 제외가 아닌 기존 가입자에게 누락된 앱 가입 추첨권만 채운다.
