    ...
    function supabasePhotoUrl(value) {
      ...
      if (raw.startsWith('admin_manual://')) return '';
    ...
    function handleInvestigationMapMissing(img) {
      ...
      img.parentElement?.querySelectorAll('.investigation-map-marker').forEach(marker => marker.classList.add('hidden'));
    ...
    function handleInvestigationMapLoaded(img) {
      ...
      img.parentElement?.querySelectorAll('.investigation-map-marker').forEach(marker => marker.classList.remove('hidden'));
      const empty = img.parentElement?.querySelector('.investigation-map-empty');
    ...
    async function loadBBB(silent = false, forceRefresh = false) {
      ...
      try {
        ...
        if (bbbRes.myPhoto || bbbRes.m1ApprovalStatus || bbbRes.m1Rewarded) {
    ...
    function _bbbInitMission2(bbbRes) {
      ...
      if (bbbRes.myPhotoM2 || bbbRes.m2ApprovalStatus || bbbRes.m2Rewarded) {
